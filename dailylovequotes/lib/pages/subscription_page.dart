...
class _SubscriptionPageState extends ConsumerState<SubscriptionPage> {
  ...
  Future<void> _initInAppPurchase() async {
    ...
    const Set<String> kProductIds = <String>{
      'com.civisolo.dailylovequotes.premium_monthly',
      'com.civisolo.dailylovequotes.premium_yearly',
    };
    ...
    try {
      ...
      setState(() {
        ...
        _subscriptionPlans =
            productDetailResponse.productDetails.map((product) {
  ...
  Future<void> _subscribe() async {
    ...
    //   if (mounted) {
    //     ScaffoldMessenger.of(context).showSnackBar(
    //       SnackBar(content: Text('subscription.ios_only'.tr())),
    //     );
    //   }
    //   return;
    // }
    ...
    try {
      if (Platform.isIOS) {
        ...
        // 根据选择的计划ID找到对应的产品
        if (_selectedPlanId!.contains('monthly')) {
          productToBuy = _products.firstWhere(
            (product) => product.id == 'com.civisolo.dailylovequotes.premium_monthly',
            orElse: () => throw Exception('未找到月度订阅产品'),
          );
        } else if (_selectedPlanId!.contains('yearly')) {
          productToBuy = _products.firstWhere(
            (product) => product.id == 'com.civisolo.dailylovequotes.premium_yearly',
            orElse: () => throw Exception('未找到年度订阅产品'),
          );
        }
        ...
        // 在Android上使用模拟订阅
        final success = await SubscriptionService.subscribe(_selectedPlanId!);
        if (success && mounted) {
          // 更新用户的订阅状态
