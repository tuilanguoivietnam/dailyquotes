...
import 'package:dailybible/config/api_config.dart';
...
class _HomePageState extends ConsumerState<HomePage>
  ...
  void _loadInitialData() async {
    ...
    // 不管是网络加载还是本地加载完成后，都再次确保通知圣经在第一位
    if (notificationAffirmation != null) {
      ref
          .read(affirmationListProvider.notifier)
          .addNotificationAffirmation(notificationAffirmation);
    ...
    // 加载其他数据
    ref.read(whitenoiseProvider.notifier).fetchWhiteNoises();
  ...
  void _initDefaultNotification() async {
    // 初始化通知服务
    ...
    if (hasPermission) {
      ...
      final time = (settings != null && settings['time'] is String)
  ...
  Future<void> _checkAndShowRatingDialog() async {
    // 在正面操作（如收藏喜欢的圣经或听完音频）之后的某个时机调用此方法
