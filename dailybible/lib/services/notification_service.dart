...
import 'package:dailybible/providers/selected_category_provider.dart';
...
class NotificationService {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      // Fallback to UTC if flutter_timezone plugin is not available
  ...
  Future<void> scheduleMultipleNotifications(String time,
    ...
    try {
      ...
      for (int hour = startHour; hour <= 22; hour += 2) {
        // 每个时间点随机选择不同的圣经
        final randomIndex = (DateTime.now().millisecondsSinceEpoch + hour) %
        ...
        await _scheduleDaily(
          ...
          notificationId: hour, // 使用小时作为通知ID
        );
      }
      ...
      print('成功设置通知，时间: $time, 通知数量: ${(22 - startHour) ~/ 2 + 1}');
    } catch (e) {
      print('设置通知失败: $e');
      rethrow;
    }
